"""
Death Counter - Single File Installer
This is a self-extracting installer that contains all necessary files.
"""

# CRITICAL: Single instance check BEFORE any imports
# This must be the absolute first thing that happens
import sys
import os
import tempfile

# Mutex is PRIMARY protection (atomic, prevents race conditions)
# File lock is secondary/backup
_installer_lock_file = None
_installer_mutex_handle = None

try:
    # Create mutex FIRST (before file lock) to prevent race conditions
    # Use a more robust approach: try OpenMutex first, then CreateMutex
    try:
        import ctypes
        from ctypes import wintypes
        
        mutex_name = "Global\\DeathCounterInstallerMutex"
        kernel32 = ctypes.windll.kernel32
        
        # Declare function signatures for proper error handling
        kernel32.GetLastError.argtypes = []
        kernel32.GetLastError.restype = wintypes.DWORD
        kernel32.OpenMutexW.argtypes = [wintypes.DWORD, wintypes.BOOL, wintypes.LPCWSTR]
        kernel32.OpenMutexW.restype = wintypes.HANDLE
        kernel32.CreateMutexW.argtypes = [ctypes.c_void_p, wintypes.BOOL, wintypes.LPCWSTR]
        kernel32.CreateMutexW.restype = wintypes.HANDLE
        
        ERROR_ALREADY_EXISTS = 183
        SYNCHRONIZE = 0x00100000
        
        # Step 1: Try to open existing mutex (fast check)
        existing_mutex = kernel32.OpenMutexW(SYNCHRONIZE, False, mutex_name)
        if existing_mutex:
            # Mutex exists - another instance is running
            kernel32.CloseHandle(existing_mutex)
            os._exit(0)
        
        # Step 2: Mutex doesn't exist, create it with initial ownership
        # Use bInitialOwner=True so we own it immediately
        kernel32.SetLastError(0)
        _installer_mutex_handle = kernel32.CreateMutexW(None, True, mutex_name)
        
        # Step 3: Check if we successfully created it or if it already existed
        # GetLastError must be called immediately after CreateMutexW
        last_error = kernel32.GetLastError()
        
        if last_error == ERROR_ALREADY_EXISTS:
            # Race condition: another process created the mutex between OpenMutex and CreateMutex
            if _installer_mutex_handle:
                kernel32.CloseHandle(_installer_mutex_handle)
                _installer_mutex_handle = None
            os._exit(0)
        
        if not _installer_mutex_handle:
            # Failed to create mutex (unexpected error)
            os._exit(1)
            
        # Success: we created and own the mutex
        # Note: We keep bInitialOwner=True, so we own it. We'll release it on exit.
            
    except Exception as e:
        # If we can't create mutex, exit immediately
        os._exit(1)
    
    # File lock as secondary/backup protection (only if mutex succeeded)
    try:
        temp_dir = tempfile.gettempdir()
        lock_file_path = os.path.join(temp_dir, "DeathCounterInstaller.lock")
        
        if sys.platform == 'win32':
            import msvcrt
            lock_file = open(lock_file_path, 'w')
            try:
                # Try to acquire exclusive lock (non-blocking)
                msvcrt.locking(lock_file.fileno(), msvcrt.LK_NBLCK, 1)
                # Write our PID
                lock_file.write(str(os.getpid()))
                lock_file.flush()
                _installer_lock_file = lock_file
            except IOError:
                # Lock failed (shouldn't happen if mutex worked, but cleanup anyway)
                lock_file.close()
                try:
                    os.remove(lock_file_path)
                except:
                    pass
        else:
            # Unix-like system
            import fcntl
            lock_file = open(lock_file_path, 'w')
            try:
                fcntl.flock(lock_file.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
                lock_file.write(str(os.getpid()))
                lock_file.flush()
                _installer_lock_file = lock_file
            except IOError:
                lock_file.close()
                try:
                    os.remove(lock_file_path)
                except:
                    pass
    except (OSError, IOError):
        # File lock failed, but mutex already protects us, so continue
        pass
        
except Exception:
    # If we can't create locks, exit immediately
    if _installer_mutex_handle:
        try:
            import ctypes
            ctypes.windll.kernel32.CloseHandle(_installer_mutex_handle)
        except:
            pass
    os._exit(1)

# Now safe to import other modules
import base64
import subprocess
import shutil
import tempfile
from pathlib import Path
import tkinter as tk
from tkinter import ttk, messagebox, scrolledtext
import threading
import webbrowser

# Embedded files (base64 encoded)
EMBEDDED_FILES = {
  "multi_game_death_counter.py": "",
  "death_counter_gui.py": "",
  "switch_game_manual.py": "",
  "games_config.json": "",
  "START_DEATH_COUNTER.bat": "",
  "STOP_DEATH_COUNTER.bat": "",
  "install_dependencies.bat": "",
  "README.md": "",
  "deathcounteraction.cs": ""
}

# Get the directory where the installer is located (works for both .exe and .py)
def get_install_dir():
    """Get the installation directory - same folder as the installer executable."""
    if getattr(sys, 'frozen', False):
        # Running as compiled .exe - use the directory where .exe is located
        return os.path.dirname(os.path.abspath(sys.executable))
    else:
        # Running as script - use the directory where the script is located
        return os.path.dirname(os.path.abspath(__file__))

INSTALL_DIR = get_install_dir()

def extract_files():
    """Extract all embedded files."""
    extracted = []
    os.makedirs(INSTALL_DIR, exist_ok=True)
    
    for filename, content_b64 in EMBEDDED_FILES.items():
        try:
            filepath = os.path.join(INSTALL_DIR, filename)
            
            # Special handling for Streamer.bot action files (.cs files for import)
            # These must remain as base64-encoded text files, NOT decoded to binary
            # Streamer.bot expects .cs files to be plain text containing base64 data
            if filename.endswith('.cs'):
                # Write as text file with base64 content (for Streamer.bot import)
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(content_b64)
            else:
                # Regular files: decode base64 and write as binary
                content = base64.b64decode(content_b64)
                with open(filepath, 'wb') as f:
                    f.write(content)
            
            extracted.append(filepath)
            print(f"Extracted: {filename}")
        except Exception as e:
            print(f"Error extracting {filename}: {e}")
    
    return extracted

def find_python_executable():
    """Find the Python executable to use for launching scripts.
    When running as frozen .exe, sys.executable points to the .exe itself,
    so we need to find the actual Python interpreter."""
    # If running as script (not frozen), use sys.executable
    if not getattr(sys, 'frozen', False):
        return sys.executable
    
    # If frozen, we need to find Python on the system
    # Try common Python executable names
    python_names = ['python', 'python3', 'pythonw', 'pythonw3']
    
    for python_name in python_names:
        try:
            result = subprocess.run([python_name, '--version'], 
                                  capture_output=True, text=True, timeout=5)
            if result.returncode == 0:
                return python_name
        except:
            continue
    
    # Try common installation paths
    common_paths = [
        r"C:\Python312\python.exe",
        r"C:\Python311\python.exe",
        r"C:\Python310\python.exe",
        r"C:\Python39\python.exe",
        r"C:\Program Files\Python312\python.exe",
        r"C:\Program Files\Python311\python.exe",
        r"C:\Program Files\Python310\python.exe",
        os.path.expanduser(r"~\AppData\Local\Programs\Python\Python312\python.exe"),
        os.path.expanduser(r"~\AppData\Local\Programs\Python\Python311\python.exe"),
        os.path.expanduser(r"~\AppData\Local\Programs\Python\Python310\python.exe"),
    ]
    
    for path in common_paths:
        if os.path.exists(path):
            try:
                result = subprocess.run([path, '--version'], 
                                      capture_output=True, text=True, timeout=5)
                if result.returncode == 0:
                    return path
            except:
                continue
    
    # Fallback: return None and let caller handle it
    return None

def check_python():
    """Check if Python is installed."""
    python_exe = find_python_executable()
    if python_exe is None:
        return False, None
    
    try:
        result = subprocess.run([python_exe, '--version'], 
                              capture_output=True, text=True, timeout=5)
        return True, result.stdout.strip()
    except:
        return False, None

def check_tesseract():
    """Check if Tesseract OCR is installed."""
    default_path = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    if os.path.exists(default_path):
        return True, default_path
    try:
        result = subprocess.run(['tesseract', '--version'], 
                              capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            return True, "tesseract"
    except:
        pass
    return False, None

def check_dependencies():
    """Check if Python dependencies are installed in system Python."""
    python_exe = find_python_executable()
    if python_exe is None:
        return False, "Python not found"
    
    try:
        # Check if packages are installed by trying to import them
        packages_to_check = ['mss', 'PIL', 'cv2', 'pytesseract', 'numpy', 'psutil']
        result = subprocess.run([python_exe, '-c', 
                               'import sys; ' + 
                               'import mss; import PIL; import cv2; import pytesseract; import numpy; import psutil; ' +
                               'sys.exit(0)'],
                              capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            return True, "All dependencies installed"
        else:
            error_msg = result.stderr.strip() if result.stderr else "Import failed"
            return False, error_msg
    except subprocess.TimeoutExpired:
        return False, "Timeout checking dependencies"
    except Exception as e:
        return False, str(e)

def install_dependencies():
    """Install Python dependencies."""
    python_exe = find_python_executable()
    if python_exe is None:
        return False, "Python not found"

    try:
        # Check Python version first - Python 3.14 is too new and doesn't have package support yet
        version_result = subprocess.run([python_exe, '--version'], capture_output=True, text=True, timeout=5)
        python_version_str = version_result.stdout.strip() if version_result.returncode == 0 else ""
        is_python314 = '3.14' in python_version_str
        
        if is_python314:
            return False, ("Python 3.14 is not yet supported by required packages (numpy, opencv-python).\n\n"
                          "Please install Python 3.12.1 (recommended - same version used to build this installer):\n"
                          "Direct download: https://www.python.org/ftp/python/3.12.1/python-3.12.1-amd64.exe\n"
                          "Or visit: https://www.python.org/downloads/\n\n"
                          "Python 3.12.1 is the version this installer was built and tested with.\n"
                          "IMPORTANT: Check 'Add Python to PATH' during installation!\n"
                          "After installing Python 3.12.1, restart this installer.")
        
        packages = ['mss', 'pillow', 'pytesseract', 'opencv-python', 'numpy', 'psutil']
        
        # Upgrade pip first
        pip_result = subprocess.run([python_exe, '-m', 'pip', 'install', '--upgrade', 'pip', 'setuptools', 'wheel'], 
                      check=False, capture_output=True, text=True, timeout=60)
        
        # Install packages one by one to see which ones fail
        failed_packages = []
        for package in packages:
            try:
                # For numpy on Python 3.14, try installing from pre-built wheels or use a specific version
                if package == 'numpy' and is_python314:
                    # Try installing numpy with --only-binary to avoid compilation issues
                    install_cmd = [python_exe, '-m', 'pip', 'install', '--only-binary=:all:', 'numpy']
                else:
                    install_cmd = [python_exe, '-m', 'pip', 'install', package]
                
                install_result = subprocess.run(install_cmd, 
                              check=False, capture_output=True, text=True, timeout=120)
                
                if install_result.returncode != 0:
                    error_output = install_result.stderr if install_result.stderr else install_result.stdout
                    failed_packages.append(f"{package}: {error_output[:200]}")
                    # Continue with other packages even if one fails
                    continue
            except subprocess.TimeoutExpired:
                failed_packages.append(f"{package}: Installation timeout")
                continue
            except Exception as e:
                failed_packages.append(f"{package}: {str(e)[:200]}")
                continue
        
        if failed_packages:
            error_msg = f"Some packages failed to install:\n" + "\n".join(failed_packages[:5])
            if len(failed_packages) > 5:
                error_msg += f"\n... and {len(failed_packages) - 5} more failures"
            return False, error_msg
        
        # Verify all dependencies are actually installed and importable
        verify_result = check_dependencies()
        if not verify_result[0]:
            return False, f"Installation completed but verification failed:\n{verify_result[1]}\n\nPlease try running 'Install Dependencies' again."
        
        # Save the Python path used for installation so GUI can use the same one
        python_path_file = os.path.join(INSTALL_DIR, "python_path.txt")
        try:
            # Convert to absolute path if it's a command name
            if not os.path.isabs(python_exe):
                # It's a command name like 'python', get full path
                full_path_result = subprocess.run([python_exe, '-c', 'import sys; print(sys.executable)'], 
                                                capture_output=True, text=True, timeout=5)
                if full_path_result.returncode == 0:
                    python_exe = full_path_result.stdout.strip()
            
            # Save the absolute path
            with open(python_path_file, 'w', encoding='utf-8') as f:
                f.write(python_exe)
        except Exception as e:
            # Non-critical, but log it for debugging
            print(f"Warning: Could not save Python path: {e}")
        
        return True, "Dependencies installed and verified successfully"
    except subprocess.TimeoutExpired:
        return False, "Installation timeout - packages may take a while to install"
    except Exception as e:
        return False, str(e)

class InstallerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Death Counter - Installer & Launcher")
        self.root.geometry("950x650")  # Made wider to fit all buttons
        self.root.resizable(True, True)  # Allow resizing in case needed
        self.root.minsize(900, 600)  # Ensure minimum size
        
        self.setup_complete = False
        self.streamerbot_imported = False  # Track if Streamer.bot action is confirmed imported
        self.launching = False  # Prevent multiple simultaneous launches
        self.create_ui()
        self.check_system()
    
    def create_ui(self):
        main_frame = ttk.Frame(self.root, padding="20")
        main_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        title = ttk.Label(main_frame, text="Death Counter", 
                         font=("Arial", 18, "bold"))
        title.grid(row=0, column=0, columnspan=2, pady=(0, 20))
        
        status_frame = ttk.LabelFrame(main_frame, text="System Check", padding="10")
        status_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        
        self.python_status = ttk.Label(status_frame, text="Checking Python...")
        self.python_status.grid(row=0, column=0, sticky=tk.W, pady=2)
        
        self.tesseract_status = ttk.Label(status_frame, text="Checking Tesseract OCR...")
        self.tesseract_status.grid(row=1, column=0, sticky=tk.W, pady=2)
        
        self.deps_status = ttk.Label(status_frame, text="Checking dependencies...")
        self.deps_status.grid(row=2, column=0, sticky=tk.W, pady=2)
        
        log_frame = ttk.LabelFrame(main_frame, text="Log", padding="10")
        log_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        main_frame.rowconfigure(2, weight=1)
        
        self.log_text = scrolledtext.ScrolledText(log_frame, height=10, width=60)
        self.log_text.pack(fill=tk.BOTH, expand=True)
        
        # Create button frames for two rows
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=3, column=0, columnspan=2, pady=(0, 10))
        
        # First row of buttons
        button_row1 = ttk.Frame(button_frame)
        button_row1.pack(fill=tk.X, pady=(0, 5))
        
        self.install_button = ttk.Button(button_row1, text="Install/Update Files", 
                                         command=self.install_files, width=20)
        self.install_button.pack(side=tk.LEFT, padx=5)
        
        self.install_python_button = ttk.Button(button_row1, text="Install Python", 
                                                command=self.install_python, width=18)
        self.install_python_button.pack(side=tk.LEFT, padx=5)
        
        self.install_tesseract_button = ttk.Button(button_row1, text="Install Tesseract", 
                                                   command=self.install_tesseract, width=18)
        self.install_tesseract_button.pack(side=tk.LEFT, padx=5)
        
        self.install_deps_button = ttk.Button(button_row1, text="Install Dependencies", 
                                              command=self.install_deps, width=18)
        self.install_deps_button.pack(side=tk.LEFT, padx=5)
        
        # Second row of buttons
        button_row2 = ttk.Frame(button_frame)
        button_row2.pack(fill=tk.X)
        
        self.streamerbot_download_button = ttk.Button(button_row2, text="Streamer.bot Download", 
                                                      command=self.download_streamerbot, width=20)
        self.streamerbot_download_button.pack(side=tk.LEFT, padx=5)
        
        self.streamerbot_button = ttk.Button(button_row2, text="Import to Streamer.bot", 
                                             command=self.import_streamerbot, width=20)
        self.streamerbot_button.pack(side=tk.LEFT, padx=5)
        
        self.streamerbot_confirm_button = ttk.Button(button_row2, text="✓ Confirm Import", 
                                                     command=self.confirm_streamerbot_import, width=18, state=tk.DISABLED)
        self.streamerbot_confirm_button.pack(side=tk.LEFT, padx=5)
        
        self.launch_button = ttk.Button(button_row2, text="Launch Death Counter", 
                                       command=self.launch_app, width=18, state=tk.DISABLED)
        self.launch_button.pack(side=tk.LEFT, padx=5)
        
        info_text = ("1. Click 'Install/Update Files' to extract all files\n"
                    "2. Click 'Install Dependencies' to install Python packages\n"
                    "3. Install Tesseract OCR from: https://github.com/UB-Mannheim/tesseract/wiki\n"
                    "4. Click 'Streamer.bot Download' to download and install Streamer.bot\n"
                    "5. Click 'Import to Streamer.bot' and import the action file, then click 'Confirm Import'\n"
                    "6. Click 'Launch Death Counter' to start")
        info_label = ttk.Label(main_frame, text=info_text, justify=tk.LEFT)
        info_label.grid(row=4, column=0, columnspan=2, sticky=tk.W)
    
    def log(self, message):
        self.log_text.insert(tk.END, message + "\n")
        self.log_text.see(tk.END)
        self.root.update()
    
    def check_system(self):
        python_ok, python_version = check_python()
        if python_ok:
            self.python_status.config(text=f"✓ Python: {python_version}", foreground="green")
        else:
            self.python_status.config(text="✗ Python not found", foreground="red")
        
        tesseract_ok, tesseract_path = check_tesseract()
        if tesseract_ok:
            self.tesseract_status.config(text="✓ Tesseract OCR: Found", foreground="green")
        else:
            self.tesseract_status.config(text="✗ Tesseract OCR: Not found (install from link below)", foreground="orange")
        
        # Check dependencies using system Python (not bundled Python)
        deps_ok, deps_msg = check_dependencies()
        if deps_ok:
            self.deps_status.config(text="✓ Dependencies: Installed", foreground="green")
        else:
            self.deps_status.config(text="✗ Dependencies: Not installed", foreground="red")
        
        gui_path = os.path.join(INSTALL_DIR, "death_counter_gui.py")
        if os.path.exists(gui_path):
            self.setup_complete = True
            self.update_launch_button_state()
            self.log("Files already installed. Import to Streamer.bot to enable launch button.")
        else:
            self.log("Files not yet installed. Click 'Install/Update Files' first.")
    
    def install_files(self):
        self.log("Installing files...")
        self.install_button.config(state=tk.DISABLED)
        
        def do_install():
            try:
                extracted = extract_files()
                self.log(f"Extracted {len(extracted)} files successfully!")
                self.setup_complete = True
                self.update_launch_button_state()
                self.log("Files extracted! Now import to Streamer.bot to enable launch button.")
            except Exception as e:
                self.log(f"Error installing files: {e}")
                messagebox.showerror("Error", f"Failed to install files: {e}")
            finally:
                self.install_button.config(state=tk.NORMAL)
        
        threading.Thread(target=do_install, daemon=True).start()
    
    def install_deps(self):
        self.log("Installing dependencies...")
        self.install_deps_button.config(state=tk.DISABLED)
        
        def do_install():
            try:
                success, message = install_dependencies()
                if success:
                    self.log("Dependencies installed successfully!")
                    self.check_system()
                else:
                    self.log(f"Failed to install dependencies: {message}")
                    messagebox.showerror("Error", f"Failed to install dependencies.\n\n{message[:300]}")
            except Exception as e:
                error_msg = str(e)
                self.log(f"Error: {error_msg}")
                messagebox.showerror("Error", f"Failed to install dependencies: {error_msg}")
            finally:
                self.install_deps_button.config(state=tk.NORMAL)
        
        threading.Thread(target=do_install, daemon=True).start()
    
    def install_python(self):
        """Open Python download page in browser."""
        self.log("Opening Python download page...")
        webbrowser.open("https://www.python.org/downloads/")
        self.log("Please install Python and ensure 'Add Python to PATH' is checked during installation.")
        messagebox.showinfo("Install Python", 
                          "Python download page opened in browser.\n\n"
                          "IMPORTANT: Check 'Add Python to PATH' during installation!\n\n"
                          "After installing, restart this installer.")
    
    def install_tesseract(self):
        """Open Tesseract OCR download page in browser."""
        self.log("Opening Tesseract OCR download page...")
        webbrowser.open("https://github.com/UB-Mannheim/tesseract/wiki")
        self.log("Please install Tesseract OCR. Default path: C:\\Program Files\\Tesseract-OCR\\")
        messagebox.showinfo("Install Tesseract OCR", 
                          "Tesseract OCR download page opened in browser.\n\n"
                          "Default install path: C:\\Program Files\\Tesseract-OCR\\\n\n"
                          "After installing, restart this installer.")
    
    def download_streamerbot(self):
        """Open Streamer.bot download page in browser."""
        self.log("Opening Streamer.bot download page...")
        webbrowser.open("https://streamer.bot/")
        self.log("Streamer.bot download page opened. Please download and install Streamer.bot.")
        messagebox.showinfo("Streamer.bot Download", 
                          "Streamer.bot download page opened in browser.\n\n"
                          "Please download and install Streamer.bot.\n\n"
                          "After installing, use the 'Import to Streamer.bot' button\nto import the action file.")
    
    def import_streamerbot(self):
        """Open file location and provide instructions for importing to Streamer.bot."""
        cs_file_path = os.path.join(INSTALL_DIR, "deathcounteraction.cs")
        
        if not os.path.exists(cs_file_path):
            messagebox.showerror("File Not Found", 
                               "The action file has not been extracted yet.\n\n"
                               "Please click 'Install/Update Files' first to extract all files.")
            return
        
        # Open the file location in Explorer
        try:
            subprocess.Popen(f'explorer /select,"{cs_file_path}"')
            self.log(f"Opened file location: {cs_file_path}")
        except Exception as e:
            self.log(f"Could not open file location: {e}")
            # Fallback: just open the directory
            try:
                os.startfile(INSTALL_DIR)
            except:
                pass
        
        # Show instructions
        instructions = (
            "Import Instructions:\n\n"
            "1. The file location has been opened in Windows Explorer\n"
            "2. Open Streamer.bot\n"
            "3. In Streamer.bot, go to Actions → Import\n"
            "4. Select the file: deathcounteraction.cs\n"
            "5. Click 'Confirm Import' button below after importing\n\n"
            "The Launch Death Counter button will be enabled after confirmation."
        )
        
        messagebox.showinfo("Import to Streamer.bot", instructions)
        self.log("Streamer.bot import instructions displayed.")
        self.log("After importing the action file, click 'Confirm Import' button.")
        
        # Enable the confirm button
        self.streamerbot_confirm_button.config(state=tk.NORMAL)
    
    def confirm_streamerbot_import(self):
        """Confirm that Streamer.bot action has been imported."""
        self.streamerbot_imported = True
        self.streamerbot_confirm_button.config(state=tk.DISABLED, text="✓ Imported")
        self.update_launch_button_state()
        self.log("Streamer.bot import confirmed! Launch button enabled.")
        messagebox.showinfo("Import Confirmed", 
                          "Streamer.bot action import confirmed!\n\n"
                          "You can now launch the Death Counter.")
    
    def update_launch_button_state(self):
        """Update the launch button state based on setup completion and Streamer.bot import."""
        if self.setup_complete and self.streamerbot_imported:
            self.launch_button.config(state=tk.NORMAL)
        else:
            self.launch_button.config(state=tk.DISABLED)
    
    def is_gui_running(self):
        """Check if GUI is already running by checking for the mutex."""
        try:
            import ctypes
            mutex_name = "Global\\DeathCounterGUIMutex"
            kernel32 = ctypes.windll.kernel32
            test_mutex = kernel32.OpenMutexW(0x00100000, False, mutex_name)  # SYNCHRONIZE access
            if test_mutex:
                kernel32.CloseHandle(test_mutex)
                return True
            return False
        except:
            # Fallback: check for python processes running the GUI
            try:
                import psutil
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        cmdline = proc.info['cmdline']
                        if cmdline and any('death_counter_gui.py' in str(arg) for arg in cmdline):
                            return True
                    except:
                        continue
            except:
                pass
            return False
    
    def launch_app(self):
        # Prevent multiple simultaneous launches
        if self.launching:
            return
        
        # Check if GUI is already running
        if self.is_gui_running():
            messagebox.showinfo("Already Running", "Death Counter GUI is already running!")
            return
        
        gui_path = os.path.join(INSTALL_DIR, "death_counter_gui.py")
        if not os.path.exists(gui_path):
            messagebox.showerror("Error", "Death Counter GUI not found. Please install files first.")
            return
        
        # Find Python executable (don't use sys.executable when frozen)
        python_exe = find_python_executable()
        if python_exe is None:
            messagebox.showerror("Error", "Python not found. Please install Python and add it to PATH.")
            return
        
        # Disable button and set launching flag
        self.launching = True
        self.launch_button.config(state=tk.DISABLED)
        
        try:
            # Launch GUI - DON'T use CREATE_NO_WINDOW so the window appears
            # Capture stderr to see errors if it fails
            # IMPORTANT: Clear TCL_LIBRARY and TK_LIBRARY env vars to prevent PyInstaller's bundled Tcl/Tk
            # from interfering with system Python's tkinter
            env = os.environ.copy()
            env.pop('TCL_LIBRARY', None)
            env.pop('TK_LIBRARY', None)
            
            process = subprocess.Popen([python_exe, gui_path], 
                                   cwd=INSTALL_DIR,
                                   stdout=subprocess.DEVNULL,
                                   stderr=subprocess.PIPE,
                                   env=env)
            # Give it a moment to start and acquire mutex
            import time
            time.sleep(1.0)  # Increased delay to allow GUI to initialize
            
            # Check if process is still running (if it crashed immediately, it won't be)
            if process.poll() is not None:
                # Process exited - it crashed
                stderr_output = ""
                try:
                    if process.stderr:
                        stderr_output = process.stderr.read().decode('utf-8', errors='ignore')
                except:
                    pass
                error_msg = f"GUI process exited immediately. Error: {stderr_output[:200] if stderr_output else 'Unknown error'}"
                self.log(f"ERROR: {error_msg}")
                if stderr_output:
                    self.log(f"Full error output: {stderr_output}")
                messagebox.showerror("Launch Failed", 
                                    f"Death Counter GUI failed to start.\n\n"
                                    f"Error: {stderr_output[:500] if stderr_output else 'Process exited immediately'}\n\n"
                                    f"Check the log for details.")
                return
            
            self.log("Death Counter launched!")
            # Don't show popup - the GUI window itself is the confirmation
        except Exception as e:
            self.log(f"Error launching: {e}")
            messagebox.showerror("Error", f"Failed to launch: {e}")
        finally:
            # Re-enable button after a short delay to prevent rapid clicking
            self.root.after(2000, self._re_enable_launch_button)  # Increased delay
    
    def _re_enable_launch_button(self):
        """Re-enable the launch button after a delay."""
        self.launching = False
        self.update_launch_button_state()

def main():
    # Mutex already created at module level - just verify it exists
    global _installer_mutex_handle
    if _installer_mutex_handle is None:
        # This shouldn't happen if module-level protection worked, but exit if it did
        os._exit(1)
    
    root = tk.Tk()
    app = InstallerGUI(root)
    
    cleanup_called = False
    
    def cleanup():
        """Cleanup on exit."""
        nonlocal cleanup_called
        if cleanup_called:
            return  # Prevent double cleanup
        cleanup_called = True
        
        global _installer_mutex_handle, _installer_lock_file
        try:
            if _installer_lock_file:
                _installer_lock_file.close()
                lock_file_path = os.path.join(tempfile.gettempdir(), "DeathCounterInstaller.lock")
                if os.path.exists(lock_file_path):
                    os.remove(lock_file_path)
        except:
            pass
        try:
            if _installer_mutex_handle:
                import ctypes
                ctypes.windll.kernel32.CloseHandle(_installer_mutex_handle)
                _installer_mutex_handle = None
        except:
            pass
    
    def on_closing():
        """Handle window closing."""
        cleanup()
        try:
            root.destroy()
        except:
            pass  # Window may already be destroyed
    
    root.protocol("WM_DELETE_WINDOW", on_closing)
    
    try:
        root.mainloop()
    finally:
        cleanup()  # Ensure cleanup happens even if mainloop exits unexpectedly

if __name__ == "__main__":
    main()
