"""
Multi-Game Soulsborne Death Counter GUI Application
...
if not getattr(sys, 'frozen', False):
    try:
        ...
        try:
            import traceback
            # Only use __file__ if it exists (not in all execution contexts)
            try:
                base_dir = os.path.dirname(os.path.abspath(__file__))
            except:
                base_dir = os.getcwd()
            error_log = os.path.join(base_dir, 'tcl_tk_detection_error.log')
...
def get_base_dir():
    ...
    if getattr(sys, 'frozen', False):
        ...
        try:
            ...
            # __file__ not available (e.g., in some interactive contexts)
...
class DeathCounterGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Multi-Game Soulsborne Death Counter")
        ...
        self.last_game_switch_time = 0  # Track when manual switch happened to prevent race conditions
    ...
    def create_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Multi-Game Soulsborne Death Counter", font=("Arial", 14, "bold"))
        ...
        control_frame.grid(row=3, column=0, columnspan=2, sticky=(W, E), pady=(0, 10))
        ...
        # Settings button
        self.settings_button = ttk.Button(main_frame, text="Settings", command=self.open_settings, width=40)
        self.settings_button.grid(row=4, column=0, columnspan=2, pady=(0, 0))
    ...
    def get_current_game(self):
        ...
        try:
            # Try state file first (most up-to-date)
            if os.path.exists(STATE_JSON):
                try:
                    with open(STATE_JSON, "r", encoding="utf-8") as f:
                        state = json.load(f)
                    game = state.get("current_game")
                    # Check if game is valid (exists in available games or is None/empty)
                    if game and game in (self.available_games if hasattr(self, 'available_games') and self.available_games else []):
                        return game
                    elif game:
                        # Game exists in state but not in available games list - return it anyway
                        # (might be a newly added game that GUI hasn't reloaded)
                        return game
                except (json.JSONDecodeError, IOError):
                    # File might be corrupted or locked, try config instead
                    pass
            ...
            if os.path.exists(CONFIG_FILE):
                try:
                    with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                        config = json.load(f)
                    game = config.get("current_game")
                    # Check if game is valid
                    if game and game in (self.available_games if hasattr(self, 'available_games') and self.available_games else []):
                        return game
                    elif game:
                        # Game exists in config but not in available games list - return it anyway
                        return game
                    # If no valid game in config, return first available game
                    if hasattr(self, 'available_games') and self.available_games:
                        return self.available_games[0]
                except (json.JSONDecodeError, IOError):
                    pass
        except Exception:
            # If there's any other error, fall through to default
            pass
        
        # Default: return first available game or "Unknown"
        if hasattr(self, 'available_games') and self.available_games:
            return self.available_games[0]
        return "Unknown"
    ...
    def cycle_game(self, event=None):
        ...
        # Record switch time to prevent monitoring thread from overwriting
        self.last_game_switch_time = time.time()
        ...
        try:
            ...
            if os.path.exists(switch_script):
                ...
                if result.returncode == 0:
                    # Force immediate GUI update on main thread after a brief delay to ensure files are written
                    self.root.after(50, lambda: self.update_status())
                    # Force update again after longer delay to handle any filesystem delays
                    self.root.after(200, lambda: self.update_status())
                    # Force update again to be safe
    ...
    def switch_game_direct(self, game_name):
        ...
        try:
            # Record switch time to prevent monitoring thread from overwriting
            if hasattr(self, 'last_game_switch_time'):
                self.last_game_switch_time = time.time()
            ...
            if os.path.exists(CONFIG_FILE):
                ...
                # Mark this as the manually selected game (fallback when no game is detected)
                config["manual_game"] = game_name
                with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                    json.dump(config, f, indent=2)
                    f.flush()  # Force write to disk
                    # Ensure file is synced (must be done while file is open)
                    try:
                        os.fsync(f.fileno())
                    except:
                        pass
            ...
            if os.path.exists(STATE_JSON):
                ...
                state = {
                    ...
                    "manual_game": game_name  # Mark as manually selected
                }
                with open(STATE_JSON, "w", encoding="utf-8") as f:
                    json.dump(state, f, indent=2)
                    f.flush()
            ...
            # Force immediate GUI update on main thread - multiple times to handle filesystem delays
            # Update immediately
            self.root.after(0, lambda: self.force_update_game_display(game_name))
            # Update again after short delay
            self.root.after(100, lambda: self.force_update_game_display(game_name))
            # Update again after longer delay to handle any filesystem delays
            self.root.after(300, lambda: self.update_status())
            # Final update after longer delay
            self.root.after(600, lambda: self.update_status())
            # Silently switch - no popup
        except Exception as e:
            messagebox.showerror("Error", f"Failed to switch game: {e}")
    
    def force_update_game_display(self, game_name=None):
        """Force update the game display immediately, bypassing file read delays."""
        try:
            # If game_name is provided, use it directly (faster)
            if game_name:
                current_game = game_name
            else:
            ...
            # If force update fails, fall back to regular update
            self.update_status()
    ...
    def start_daemon(self):
        ...
        try:
            ...
            # On Windows, use CREATE_NO_WINDOW to prevent PowerShell/console window from appearing
            if sys.platform == "win32":
                ...
                self.daemon_process = subprocess.Popen(
                    ...
                    shell=False  # Don't use shell to avoid extra windows
                )
            else:
                self.daemon_process = subprocess.Popen(
                    [python_cmd, script_path],
        ...
        while waited < max_wait_time:
            ...
            if self.daemon_process.poll() is not None:
                    ...
                    try:
                        ...
                        # Try to get more info by running the script directly (hidden window)
                        if sys.platform == "win32":
                            CREATE_NO_WINDOW = 0x08000000
    ...
    def start_monitoring(self):
        ...
        self.last_game_switch_time = 0  # Track when manual switch happened
        
        def monitor():
            while self.monitoring:
                try:
                    # Check if a manual switch just happened (within last 1 second)
                    # If so, skip this update to avoid race condition with file writes
                    current_time = time.time()
                    if current_time - self.last_game_switch_time > 1.0:
    ...
    def open_settings(self):
        ...
        try:
            ...
            import sys
            import os
            settings_path = os.path.join(BASE_DIR, "death_counter_settings.py")
            if os.path.exists(settings_path):
                # Add base directory to path if needed
                if BASE_DIR not in sys.path:
                    sys.path.insert(0, BASE_DIR)
                from death_counter_settings import SettingsWindow
                settings_window = SettingsWindow(self.root, BASE_DIR, CONFIG_FILE)
                settings_window.show()
            else:
                messagebox.showerror("Error", f"Settings module not found at: {settings_path}")
        except ImportError as e:
            messagebox.showerror("Error", f"Failed to import settings module: {e}\n\nMake sure death_counter_settings.py is in the same directory.")
        except Exception as e:
    ...
    def on_closing(self):
        ...
        try:
            ...
            try:
                # Try to access a window property to see if it exists
                _ = self.root.winfo_viewable()
                self.root.destroy()
...
def main():
    ...
    cleanup_called = False  # Track if cleanup has been called
    ...
    def cleanup():
        ...
        try:
            if hasattr(app, 'on_closing'):
                ...
                try:
                    # Try to access a window property to see if it exists
                    _ = root.winfo_viewable()
                    root.destroy()
                except (AttributeError, TclError):
                    pass  # Window already destroyed
        except:
            # Window might already be destroyed, that's OK
            pass
