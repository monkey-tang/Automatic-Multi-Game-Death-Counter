...
class InstallerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title(f"Death Counter Installer v{INSTALLER_VERSION} (Online)")
    ...
    def create_ui(self):
        ...
        # Welcome text
        welcome_text = f"""Welcome to the Death Counter Installer v{INSTALLER_VERSION} (Online)!

This installer will:
• Extract all necessary files ({len(EMBEDDED_FILES)} Python scripts, configs, and documentation)
• Install Python dependencies automatically via pip (requires internet connection)
• Set up configuration files
• Create desktop shortcuts
• Verify system requirements

⚠️ NOTE: This is the ONLINE version - internet connection required for dependency installation.
Click 'Check Requirements' to verify your system, then click 'Install' to proceed."""
        ...
        deps_cb = ttk.Checkbutton(options_frame, text="Install Python dependencies automatically", variable=self.install_deps_var)
        deps_cb.grid(row=1, column=0, sticky=tk.W, pady=(5, 0))
        ttk.Label(options_frame, text="(Installs required packages from requirements.txt)", font=("Arial", 8), foreground="gray").grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=(5, 0))
