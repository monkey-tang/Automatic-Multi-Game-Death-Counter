...
A fully automatic, reliable death counter for Dark Souls, Elden Ring, Sekiro, and other Soulsborne games. Supports **three detection methods**: OCR (Optical Character Recognition), Log File Monitoring, and Process Memory Scanning.

## 🎮 Supported Games

All PC Soulsbourne games that display red "YOU DIED" text on death:

- **Elden Ring**
- **Dark Souls 3**
- **Dark Souls Remastered**
- **Dark Souls: Prepare to Die Edition** (Original)
- **Dark Souls 2** (Original)
- **Dark Souls II: Scholar of the First Sin**
- **Demon's Souls (Remake)** (PC via PS Plus Premium / streaming)
...
- **Manual Game Selection** - Choose which game to track when no game is running
- **Smart Game Switching** - Auto-switches to detected games, reverts to manual selection when game closes
...
#### Online Installer (~10MB)
- **Location:** `online-exe/DeathCounterInstaller-online.exe`
- **Requires:** Internet connection (installs dependencies via pip)
- **Best for:** Most users, smaller download size

1. **Download** `DeathCounterInstaller-online.exe` from the `online-exe/` folder
2. **Run** the installer
3. **Choose installation directory** (default: Desktop\DeathCounter)
4. **Select options:**
   - ✅ Create desktop shortcut
   - ✅ Install Python dependencies automatically (requires internet)
   - ✅ Check for Tesseract OCR
5. **Click "Install"**
6. **Wait for installation to complete**
7. **Double-click the desktop shortcut** or run `START_DEATH_COUNTER.bat`

#### Offline Installer (~71MB)
- **Location:** `offline-exe/DeathCounterInstaller-offline.exe`
- **Requires:** No internet connection (all dependencies bundled)
- **Best for:** Offline environments or when you want everything included

1. **Download** `DeathCounterInstaller-offline.exe` from the `offline-exe/` folder
2. **Run** the installer (works completely offline)
3. **Choose installation directory** (default: Desktop\DeathCounter)
4. **Select options:**
   - ✅ Create desktop shortcut
   - ✅ Check for Tesseract OCR
...
#### Online Installer
- **Download Size:** ~10 MB (installer EXE)
- **Installation Disk Space:** ~150-200 MB (after dependency installation)
  - Python scripts: ~500 KB
  - Python dependencies (via pip): ~140-190 MB
  - Configuration files: <1 MB
- **Installation Time:** 2-5 minutes (depends on internet speed)
- **Runtime Memory Usage:**
  - GUI: ~50-80 MB RAM
  - Daemon: ~80-150 MB RAM (with OCR processing)
  - Total: ~130-230 MB RAM during operation
- **CPU Usage:** Low to moderate (5-15% on average CPU during OCR processing)
- **Network Usage:** ~50-100 MB during installation (for pip dependencies)

#### Offline Installer
- **Download Size:** ~71 MB (installer EXE with all dependencies)
- **Installation Disk Space:** ~250-300 MB
  - Python scripts: ~500 KB
  - Bundled dependencies: ~240-290 MB
  - Configuration files: <1 MB
- **Installation Time:** 1-3 minutes (no internet required)
- **Runtime Memory Usage:**
  - GUI: ~50-80 MB RAM
  - Daemon: ~80-150 MB RAM (with OCR processing)
  - Total: ~130-230 MB RAM during operation
- **CPU Usage:** Low to moderate (5-15% on average CPU during OCR processing)
- **Network Usage:** None (completely offline)

**Note:** Resource usage may vary based on:
- Number of active games being monitored
- OCR processing frequency and complexity
- Screen resolution and capture region size
- Whether log monitoring or memory scanning is enabled
...
- **Game Selection** - Click to cycle through games or select from dropdown
  - Manual game selection persists when no game is running
  - Automatically switches to detected game when it starts
  - Reverts to manual selection when detected game closes
...
- **Per-Game OCR Region Settings** - Configure OCR capture region for each game
  - Support for percentage-based (0.0-1.0) or pixel-based coordinates
  - Coordinate type toggle with optional auto-conversion
  - Input validation with warnings
- **Status Indicators** - See which methods are active
- **Save/Reset** - Apply changes or reset to defaults (includes OCR regions)
- **Restart Warning** - Notifies when daemon restart is needed
- **Enhanced Error Handling** - Detailed error messages with automatic recovery
...
- **region**: Screen region to capture for OCR
  - **Percentage-based** (recommended for multi-resolution): `{"use_percentages": true, "left": 0.27, "top": 0.43, "width": 0.46, "height": 0.19}`
  - **Pixel-based** (for specific resolution): `{"use_percentages": false, "left": 520, "top": 470, "width": 880, "height": 200}`
  - Can be configured in Settings GUI for each game
- **keywords**: Death-related keywords to search for (including OCR misreadings)
...
- Adjust `region` to capture the right screen area
  - Use Settings GUI to configure per-game OCR regions
  - Choose percentage-based (0.0-1.0) for multi-resolution support
  - Or use pixel-based for specific resolution
  - Coordinate conversion available in Settings GUI
...
**Solutions:**
1. **OCR Method:**
   - Adjust capture region using Settings GUI (recommended) or `games_config.json`
   - Use Settings GUI → Per-Game OCR Region Settings for easy configuration
   - Choose percentage-based coordinates for multi-resolution support
...
4. **Manual game selection:** Click the game name in GUI to manually select which game to track
   - Manual selection persists when no game is running
   - Auto-detection will switch to detected games, but reverts to manual selection when game closes
...
**Or using Settings GUI (Recommended):**
1. Open Settings GUI
2. Scroll to "Per-Game OCR Region Settings"
3. Select your game from the dropdown
4. Choose coordinate type (percentage or pixels)
5. Adjust Left, Top, Width, Height values
6. Click "Save"
...
2. **Find the capture region:**
   ...
   - Use Settings GUI to adjust `region` coordinates (easier than editing JSON)
   - Or manually adjust `region` coordinates in config

3. **Test and refine:**
   - Start daemon and play the game
   - Check `debug.log` for OCR output
   - Adjust keywords and region as needed (use Settings GUI for regions)
...
2. **Set monitor index:**
   - In Settings GUI: Change "Default Monitor Index" (for all games)
   - Or in `games_config.json`: Set per-game `"monitor_index"` (e.g., `"monitor_index": 2` for second monitor)

3. **Per-game monitors:**
   - Each game can have its own `monitor_index` setting in `games_config.json`
   - Default monitor index applies when game-specific setting is not present
...
- ✅ Added Settings GUI for easy configuration
  - **NEW:** Per-game OCR region editing (percentage and pixel-based)
  - **NEW:** Coordinate type toggle with auto-conversion
  - **NEW:** Enhanced error handling with detailed messages
  - **NEW:** Automatic config file backup before saving
  - **NEW:** Input validation with user warnings
- ✅ Added Fuzzy OCR Matching
- ✅ Enhanced error handling and logging
  - Detailed JSON parsing errors (line/column information)
  - Graceful fallback to default configuration
  - Config file backup system
  - Save error recovery
- ✅ Added comprehensive installer (online and offline versions)
- ✅ Multi-monitor support improvements
- ✅ Better game detection logic
- ✅ Production-ready error handling
- ✅ **Fixed manual game switching**
  - Manual game selection now persists when no game is running
  - Improved state synchronization for immediate manual game changes
  - Auto-detection reverts to manual selection when detected game closes
  - File flushing and syncing ensures immediate writes
...
2. **Calibrate capture region:**
   - Use Settings GUI → Per-Game OCR Region Settings (easiest method)
   - Use debug capture to verify region
   - Adjust region to only capture "YOU DIED" area
   - Use percentage-based coordinates (0.0-1.0) for multi-resolution support
